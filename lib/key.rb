class Key
  attr_reader :scale, :key, :scale_klass, :tuning

  DEFAULT_TUNING      = EvenTuning
  DEFAULT_SCALE_KLASS = TwelveNoteScale
  DEFAULT_KEY         = "C"

  def initialize key=DEFAULT_KEY, scale=DEFAULT_SCALE_KLASS, tuning=DEFAULT_TUNING
    @key, @scale_klass, @tuning = key, scale, tuning
  ...
  attr_reader :notes_by_name, :notes_by_midi_number
